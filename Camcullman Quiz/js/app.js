...
var state = {
  ...
  // The "state" variable created above outlines its objects, which are the questions for the quiz. 
  //When you move forward to the next state, you move forward to the next question. 
  //Each object contains 3 types of info - the question itself, the potential answers, and the index determining the correct answer. 
  //Correct choice index shows which answer is correct minus 1, since 0 = the first option

  ///Questions and answers reflect ski culture knowledge, while poking fun at how some people go overboard
  questions: [
    {
      //The question itself is presented
      text: "What does it mean to shred the gnar?",
      //A number of potential choices shown
      choices: ["It means to eat a lot", "shred paper", "ski/board in an extreme fashion", "take a nap"],
      //The correct answer is determined below in the correct choice index. THe index = the correct answer -1 since that is the norm for referrring to items in array. 
  ...
  //Question starts at the first question (0=first question)
  ...
  // determines if last answer was true or false - doesn't matter which you put to start with in the below because it gets updated after the first answer the user submits. 
  lastAnswerCorrect: true,
  
  //Determines feedback given in the select feedback and render feedback sections (more on those later)
  feedbackRandom: 0
};

// State modification functions
//these modify the state as the quiz goes on (such as the state of which question the user is on, how many they've gotten correct, etc.)
//The state is only created in these functions. The functions to render the data based on these states is in the render functions section. 



//creates our "set the route" function with 2 arguments: state and route. This determines the route the quiz goes based on its current state (the question it's on)
function setRoute(state, route) {
  state.route = route;
};

// Function to restart the game (offered at the end of the game) by bringing us back to the homepage and resetting score and question index back to 0
...
//because it needs to know where the user is in the quiz and how they answered to take the proper actions after the user answer. 
//After determining the question - it looks at whether the right answer was selected and if so the score inciments 1 and the proper answer feedback page is prompted to come next. 
...
// Render functions - render functions refer to the process of displaying outputs (often called fetching data)
//In this case, we see it as which pages are shown (and which are hidden) and what properties they show. 
...
//It also shows how the choices are laid out in a list & that an answer is required for this specific radio button (more on radio buttons below)
...
//i.e. after the last question, the button that renders communicates to the user they can click it to see their final results. 
...
//This function renders the final feedback text - which tells the user how many questions they got right
...
//Here we create an object with keys (start, question, etc.) and matching values (these are JQuerry selectors). 
//This allows us to use the shorthand for the JQuerry selectors above.
//This object of page elements determines the 4 possible pages (start, question, feedback, final feedback)
...
$("form[name='game-start']").submit(function(event) {
  ...
  //This line binds the submit event to the setroute function outlined earlier (and used in many spots in the render functions)
  setRoute(state, 'question');
  //binds the submit event to the renderApp function
...
$(".restart-game").click(function(event){
  //We use prevent default here because when the user tries to submit a form in HTML, the default behavior is to submit the form to a server. In this case we don't want this, so we use prevent default.
  event.preventDefault();
  //This line states to bind to the reset game state function noted above- which puts state score at 0, question index at 0, and brings the user to the start page
  resetGame(state);
  //binds the click event to the renderApp function noted above
  renderApp(state, PAGE_ELEMENTS);
});
...
//The function that render the app based on which state it's in and the page it should be on is listed last to ensure it renders after all state modifications and renderings are complete.
